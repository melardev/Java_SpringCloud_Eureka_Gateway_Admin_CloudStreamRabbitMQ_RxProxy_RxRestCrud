...
# Introduction
This microservices app is composed of Eureka, Spring Cloud Gateway, Reactive Rest Api and a Reactive Proxy based on WebClient.
You can perform CRUD through the gateway the base addresses are:
localhost:8080/api/todos for the todo-service and<br>
localhost:8080/proxy/todos for the proxy-service<br>
both requests will go through the gateway.

When you create a Todo, the todo-service will send a message through Spring Cloud Stream, that message will end up being
received by admin-server-service.
